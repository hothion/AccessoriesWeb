...
$user = null;
	...
	if($phukien['type'] == 'gay chup anh'){
 	...
 	for($i = 0; $i < count($phukiens); $i++){
 		if($phukiens[$i]->id==$_POST["insert_cart"]){


        $id=$_POST["insert_cart"]; 
        $name1=$phukiens[$i]->name;
        $price1=$phukiens[$i]->getDisplayPrice();
        $quantity1=$phukiens[$i]->quantity;
        $image=$phukiens[$i]->getImagePath();
       	$sql = "INSERT INTO cart(cart_id, name,price,quantity,image)
				VALUES(null,'".$name1."', '".$price1."','".$quantity1."','".$image."')";
        $db->query($sql);
}
    }
}
...
<body>
	<div class="page-footer font-small special-color-dark pt-4" style="color: white; margin-top: 0px; margin-bottom: 0px;">
		...
		<button type="submit"style="border-radius: 2px;"><i class="fa fa-search"></i></button>
			...
			<div>
			<form action="cart.php" method="">
			<button onclick="cart()"style="background: black;border: none;"><i class="fas fa-shopping-cart"></i></button>
			</form>
			</div>
    ...
    <form action="" method="post">
        <div class="pk-container">
            ...
            ?>
				...
				<button name="insert_cart"value="<?php echo $phukiens[$i]->id;?>">Add</button>
			    </form>
